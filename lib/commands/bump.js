import { bumpPatch, formatVersion, getLatestTagForEnv, runGit } from '../utils.js';
    ...
    .action(async (envArg) => {
      ...
      try {
        if (message && message.trim()) {
          runGit(['tag', '-a', finalTag, '-m', message.trim()]);
        } else {
          runGit(['tag', finalTag]);
        }

        console.log(chalk.green(`✓ Created tag ${chalk.bold(finalTag)}`));

        runGit(['push', 'origin', finalTag]);
        console.log(chalk.green(`✓ Pushed ${chalk.bold(finalTag)} to origin `));
